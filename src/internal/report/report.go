...
var helpTips = map[string]string{
	...
	"TAG001":       "Use @[json], @[json \"name\"], @[json omitempty], or @[tag \"json:\\\"field\\\"\"] as the escape hatch.",
	"FLOAT001":     "Do not use = / == / <> on float; compare with a tolerance, or use std.decimal for money.",
	"STR001":       "Use strings.EqualFold a b instead of ToLower/ToUpper then compare.",
	"REGEXP001":    "Compile the regexp once, outside the loop (MustCompile at init, or hoist the value).",
	"WG001":        "Call WaitGroup.Add before go (fun () -> …), not inside the goroutine.",
	"URL001":       "Query() copies values; Set on the copy is discarded. Assign, Set, then RawQuery = q.Encode().",
	"EXIT001":      "os.Exit skips try/finally (lowered to defer). Return an error or let main finish.",
	"CTX001":       "Keep the cancel func from WithCancel/WithTimeout and call it (try/finally).",
	"MODULE001":    "Rename the duplicate binding, or use different module names / directories. module main files are never merged.",
	"LEX002":       "Rewrite each (* … *) as // to end of line.",
...
func tipForMessage(rest string) string {
	...
	ordered := []string{
		...
		"CODEGEN003", "CODEGEN002", "CODEGEN001", "GOSIG004", "GOSIG003", "GOSIG002", "GOSIG001", "TAG001", "MODULE001", "LEX002",
		"FLOAT001", "STR001", "REGEXP001", "WG001", "URL001", "EXIT001", "CTX001",
